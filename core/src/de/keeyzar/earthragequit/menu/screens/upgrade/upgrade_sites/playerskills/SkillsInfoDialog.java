 ...
 */
public class SkillsInfoDialog {
    private Stage stage;


    public SkillsInfoDialog(Stage stage){
        this.stage = stage;
    }
    ...
    private void addContent(final Dialog dialog, String text, String title) {
        ...
        contentTable.add(new Label(LANG.format("upgrade_dialog_info_dialog_title", title), skin)).pad(ScreenVariables.PAD_TB_S).row();
        contentTable.add(motherTable).grow().row();

        contentTable.add("").grow().row();


        TextButton tb = new TextButton(LANG.format("upgrade_dialog_info_dialog_close"), skin);
